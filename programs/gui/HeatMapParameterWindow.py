...
class HeatMapParameterWindow(QWidget):
    def __init__(self, parent, var_name: str, begin_date: datetime,
                 end_date: datetime, level_count: int):
        super().__init__()
        ...
        self.export_data_type_combobox.setGeometry(self.margin,
        ...
        self.submit_button.setGeometry(
            self.margin, 10 + 8.5 * self.element_height, self.button_width,
            self.element_height)

        self.setGeometry(0, 0, self.button_width + 2 * self.margin, 10 + 10 * self.element_height)
        self.center_window()
        self.show()
